...
function InfoSection({ title, items }: { title: string; items: InfoItem[] }) {
  return (
    <section className="rounded-3xl bg-background/80 p-4 ring-1 ring-border/50">
...
export function ProfilePanel() {
  ...
  return (
    <aside className="fixed bottom-0 right-0 top-0 z-50 hidden w-[320px] rounded-l-3xl border border-border/70 bg-card/95 p-6 shadow-floating backdrop-blur-xl lg:flex lg:flex-col">
      ...
      <div className="flex flex-col items-center text-center">
        ...
        <h3 className="mt-4 text-sm font-bold text-foreground">{fullName}</h3>
      ...
      <div className="scrollbar-thin mt-7 min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
      ...
      <div className="mt-5 space-y-2">
        <button
          type="button"
          className="flex w-full items-center justify-center gap-2 rounded-2xl bg-primary py-2.5 text-xs font-bold text-primary-foreground transition hover:bg-primary/90"
        >
          <Edit3 size={15} />
          Edit Profile
        </button>
