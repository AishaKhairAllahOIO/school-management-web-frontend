...
const WEBSITE_URL = "https://your-school-website.com";

function formatMobileSidebarDate(locale: string) {
  const today = new Date();

  return {
    day: new Intl.DateTimeFormat(locale, { day: "2-digit" }).format(today),
    month: new Intl.DateTimeFormat(locale, { month: "short" }).format(today),
    weekday: new Intl.DateTimeFormat(locale, { weekday: "long" }).format(today),
  };
}
  ...
  return (
    <div className="fixed inset-0 z-[80] lg:hidden">
      ...
      <aside
        ...
        className={[
          "sidebar-gradient sidebar-shell absolute top-0 z-10",
          "flex h-full w-[268px] max-w-[88vw] flex-col overflow-hidden",
          "text-sidebar-foreground",
          isRtl ? "right-0 rounded-l-[28px]" : "left-0 rounded-r-[28px]",
        ].join(" ")}
      >
        ...
        <div className="relative z-10 flex h-full min-h-0 flex-col">
          <header className="relative flex h-[104px] shrink-0 items-center justify-center px-4">
            <div aria-hidden="true" className="pointer-events-none absolute inset-x-3.5 inset-y-[11px] rounded-[18px] bg-sidebar-foreground/[0.075]" />

            <div className="relative flex min-w-0 flex-col items-center text-center">
              <div className="flex items-center justify-center gap-1.5 text-[10px] font-medium leading-[13px] text-sidebar-muted/78">
                <Sparkles aria-hidden="true" size={11} strokeWidth={1.9} />
                <span className="whitespace-nowrap">{greeting}</span>
              </div>

              <h1 className="mt-[5px] whitespace-nowrap text-[17px] font-semibold leading-[19px] tracking-[-0.02em] text-sidebar-foreground">
                {t.layout.sidebar.schoolName}
              </h1>

              <div className="mt-[6px] flex items-center justify-center gap-1.5 text-[10px] leading-[13px] text-sidebar-muted/72">
            ...
            <button
              ...
              onClick={closeMobileSidebar}
              aria-label={t.layout.sidebar.closeSidebar}
              title={t.layout.sidebar.closeSidebar}
              className="absolute end-4 top-1/2 flex h-[38px] w-[38px] -translate-y-1/2 items-center justify-center text-sidebar-muted transition-colors duration-200 hover:text-sidebar-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sidebar-foreground/25"
            >
              <PanelLeftClose
                aria-hidden="true"
                size={17}
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden px-3.5">
            <div className="-mt-[7px] mb-[5px] flex h-6 shrink-0 items-center gap-3 px-2">
              ...
              <span className="relative -top-[3px] h-px flex-1 bg-sidebar-foreground/[0.055]" />
            </div>
            ...
            <div className="shrink-0 pt-2 pb-[max(18px,env(safe-area-inset-bottom))]">
              ...
              <a
                ...
                className="group flex h-[48px] w-full items-center rounded-[17px] px-3 text-sidebar-muted transition-colors duration-200 hover:bg-sidebar-foreground/[0.045] hover:text-sidebar-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sidebar-foreground/25"
              >
