...
export function StudentReportCardModal({ 
  ...
  // جلب الداتا من الإعدادات كحل بديل (Fallback) ذكي وديناميكي
  ...
  // 🌟 سحب الداتا من الباك إند للطالب، وإذا كانت فارغة نسحبها من إعدادات النظام الحية 🌟
  const schoolName = student.school_name || generalSettings?.schoolName || "اسم المدرسة غير متوفر"; 
  
  // البحث عن العام الدراسي الحالي من الإعدادات
  const currentYearObj = academicData?.academicYears?.find(y => y.isCurrent) || academicData?.academicYears?.[0];
    ...
    <>
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-md p-4 print:hidden" dir="rtl">
        <div className="relative w-full max-w-5xl max-h-[95vh] overflow-y-auto rounded-[32px] bg-background shadow-2xl border border-border/50 text-right">
          ...
          <div className="sticky top-0 z-20 flex items-center bg-background/90 backdrop-blur-xl px-8 py-5 border-b border-border/50 rounded-t-[32px]">
            {/* الأزرار على اليمين */}
  ...
  semesterName, mentorName, principalName, schoolName, academicYear, serialNumber, isPrint = false 
    ...
    return (
        <>
            ...
            <div className={`flex flex-col items-center min-h-[70vh] bg-gradient-to-b from-primary/[0.04] to-background rounded-[24px] border border-primary/10 px-10 pt-16 pb-24 relative overflow-hidden ${isPrint ? 'print-page print:rounded-none print:border-none print:bg-none print:bg-gradient-to-b print:from-primary/[0.04] print:to-white' : ''}`}>
                
                <div className="relative z-10 flex flex-col items-center w-full max-w-2xl text-center space-y-12">
                  ...
                  <div className="w-full max-w-md bg-card print:bg-transparent rounded-[20px] border-2 border-primary/20 print:border-black p-8 shadow-sm print:shadow-none mx-auto">
                    <div className="flex flex-col gap-6 text-right">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-[17px] font-bold text-muted-foreground print:text-black whitespace-nowrap min-w-max">اسم التلميذ/ة:</span>
                        <span className="text-[20px] font-black text-foreground print:text-black text-left">{studentName}</span>
                      </div>
                      
                      <div className="flex items-center justify-between">
                        <span className="text-[17px] font-bold text-muted-foreground print:text-black whitespace-nowrap min-w-max">المدرسة:</span>
                        <span className="text-[18px] font-bold text-foreground print:text-black text-left">{schoolName}</span>
                      </div>
                      
                      <div className="flex items-center justify-between">
                        <span className="text-[17px] font-bold text-muted-foreground print:text-black whitespace-nowrap min-w-max">الصف:</span>
                        <span className="text-[18px] font-bold text-foreground print:text-black capitalize text-left">{gradeName}</span>
                      </div>
                      
                      <div className="flex items-center justify-between">
                        <span className="text-[17px] font-bold text-muted-foreground print:text-black whitespace-nowrap min-w-max">الشعبة:</span>
                        <span className="text-[18px] font-bold text-foreground print:text-black text-left">{classroomName}</span>
                      </div>
                ...
                <div className="absolute bottom-12 w-full flex flex-col items-center justify-center text-center space-y-1">
                  <span className="text-[14px] font-bold text-muted-foreground print:text-black">{semesterName}</span>
                  <div className="flex items-center gap-1 text-[13px] font-semibold text-muted-foreground print:text-black">
            ...
            <div className={`space-y-4 ${isPrint ? 'print-page pt-0' : 'pt-6'}`}>
                ...
                <div className="hidden print:flex items-center justify-between border-b-2 border-black pb-2 mb-4 mt-0">
                  ...
                  <div className="text-left text-[12px] font-bold space-y-1">
                    <p>اسم التلميذ/ة: {studentName}</p>
                    <p>الرقم التسلسلي: {serialNumber}</p>
                  </div>
                </div>

                {/* شريط ملخص الحضور والغياب المحدث */}
                <div className="flex flex-row items-center justify-between mb-4 bg-card print:bg-transparent border border-border/50 print:border-black py-2 px-8 rounded-[12px] print:rounded-none shadow-sm print:shadow-none">
                  <div className="flex flex-col items-center">
                    <div className="text-[10px] text-muted-foreground print:text-black font-semibold mb-0.5">المجموع النهائي</div>
                    <div className="text-[13px] font-black" dir="ltr">{summary.total_marks ?? '-'} <span className="text-[10px] text-muted-foreground print:text-black font-normal">/ {summary.max_total_marks ?? '-'}</span></div>
                  </div>
                  
                  <div className="flex flex-col items-center">
                    <div className="text-[10px] text-muted-foreground print:text-black font-semibold mb-0.5">النتيجة النهائية</div>
                    <div className={`inline-flex items-center justify-center gap-1 px-3 py-0.5 rounded-[4px] text-[12px] font-bold ${
                      isPassed ? 'text-success print:text-black' : 'text-destructive print:text-black'
                    }`}>
                      {isPassed ? <CheckCircle2 size={13} className="print:hidden"/> : <XCircle size={13} className="print:hidden"/>}
                      {isPassed ? 'ناجــح' : 'راســب'}
                    </div>
                  </div>
                  
                  <div className="flex flex-col items-center">
                    <div className="text-[10px] text-muted-foreground print:text-black font-semibold mb-0.5">نسبة الدوام</div>
                    <div className="text-[13px] font-bold text-primary print:text-black" dir="ltr">{summary.attendance_percentage ?? '-'}</div>
                  </div>
                  
                  <div className="flex flex-col items-center">
                    <div className="text-[10px] text-muted-foreground print:text-black font-semibold mb-0.5">غياب مبرر</div>
                    <div className="text-[13px] font-bold text-warning print:text-black">0 أيام</div>
                  </div>
                  
                  <div className="flex flex-col items-center">
                    <div className="text-[10px] text-muted-foreground print:text-black font-semibold mb-0.5">غياب غير مبرر</div>
                    <div className="text-[13px] font-bold text-destructive print:text-black">{summary.absences_count ?? '0'} أيام</div>
                  </div>
                </div>

                {summary.failure_reasons && summary.failure_reasons.length > 0 && !isPassed && (
                  <div className="rounded-[12px] print:rounded-none bg-destructive/10 print:bg-transparent border border-destructive/20 print:border-black p-3 text-destructive print:text-black space-y-1 mb-2 text-right">
                    ...
                    <ul className="list-disc list-inside space-y-1 pr-2 text-[11px] font-medium">
                ...
                <div className="overflow-hidden rounded-[12px] print:rounded-none border border-border/60 print:border-black bg-card print:bg-transparent shadow-sm print:shadow-none">
                  <table className="w-full text-center text-[10px] print:text-[11px]">
                    <thead>
                      <tr className="border-b border-border/50 print:border-black bg-primary/10 print:bg-gray-100 text-primary print:text-black">
                        <th className="px-2 py-2 text-right font-bold">المادة الدراسية</th>
                        <th className="px-1 py-2 font-bold">الشفوي</th>
                        <th className="px-1 py-2 font-bold">الوظائف</th>
                        <th className="px-1 py-2 font-bold">مذاكرة 1</th>
                        <th className="px-1 py-2 font-bold">مذاكرة 2</th>
                        <th className="px-1 py-2 font-bold">المشاركة</th>
                        <th className="px-1 py-2 font-bold">الامتحان</th>
                        <th className="px-1 py-2 font-black border-r border-border/50 print:border-black bg-primary/5 print:bg-gray-200">العلامة</th>
                        <th className="px-1 py-2 font-bold">الحد الأعلى</th>
                        <th className="px-1 py-2 font-bold text-destructive/80 print:text-black">الأدنى</th>
                      ...
                      {subjects.map((sub: any, index: number) => {
                        ...
                        return (
                          <tr key={index} className="transition-colors hover:bg-muted/30 print:hover:bg-transparent">
                            <td className="px-2 py-1.5 font-bold text-[11px] text-right">{sub.subject_name}</td>
                            <td className="px-1 py-1.5 text-muted-foreground print:text-black font-medium">{ev.oral?.mark ?? '-'}</td>
                            <td className="px-1 py-1.5 text-muted-foreground print:text-black font-medium">{ev.homework?.mark ?? '-'}</td>
                            <td className="px-1 py-1.5 text-muted-foreground print:text-black font-medium">{ev.quiz1?.mark ?? '-'}</td>
                            <td className="px-1 py-1.5 text-muted-foreground print:text-black font-medium">{ev.quiz2?.mark ?? '-'}</td>
                            <td className="px-1 py-1.5 text-muted-foreground print:text-black font-medium">{ev.participation?.mark ?? '-'}</td>
                            <td className="px-1 py-1.5 text-muted-foreground print:text-black font-medium">{ev.exam?.mark ?? '-'}</td>
                            <td className="px-1 py-1.5 font-black border-r border-border/50 print:border-black bg-primary/[0.02] print:bg-transparent text-[12px]">{sub.subject_total ?? '-'}</td>
                            <td className="px-1 py-1.5 font-bold text-foreground print:text-black">{sub.max_mark ?? '-'}</td>
                            <td className="px-1 py-1.5 font-bold text-destructive/80 print:text-black">{sub.passing_mark ?? '-'}</td>
