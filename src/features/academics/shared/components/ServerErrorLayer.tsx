...
function collectMessages(value: unknown, result: string[] = []): string[] {
  ...
  if (value && typeof value === "object") {
    ...
    if (result.length === 0 && "data" in record) {
      collectMessages(record.data, result);
    }

    if (result.length === 0) {
      Object.values(record).forEach((item) => collectMessages(item, result));
    }
  }
...
export function useServerErrorLayer() {
  ...
  useEffect(() => {
    ...
    const queryCacheUnsubscribe = queryClient.getQueryCache().subscribe((event) => {
      if (event.type === "updated" && event.query.state.error) {
        showError(event.query.state.error);
      }
    });

    const mutationCacheUnsubscribe = queryClient.getMutationCache().subscribe((event) => {
