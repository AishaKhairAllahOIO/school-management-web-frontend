...
type TeacherSchedule = Record<string, Record<string, ScheduleEntryType[]>>;
    ...
    <div className="space-y-4" style={academicTheme.style}>
      ...
      <section className="overflow-hidden rounded-[22px] border border-border/70 bg-card shadow-[0_12px_34px_rgba(31,24,74,0.055)]">
        <header className="relative overflow-hidden px-6 py-5 sm:px-7">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="inline-flex h-10 w-full shrink-0 items-center justify-center gap-2 rounded-full bg-[var(--academic-soft)] px-5 text-sm font-semibold text-[var(--academic-accent)] sm:w-auto">
      ...
      <div className="space-y-4">
        {[1, 2].map((teacher) => (
          <section
            key={teacher}
            className="overflow-hidden rounded-[20px] border border-border/70 bg-card shadow-[0_10px_30px_rgba(31,24,74,0.045)]"
          >
            ...
            <div className="hidden overflow-x-auto md:block">
              <div className="min-w-[900px]">
                {/* Table header */}
                <div className="grid grid-cols-[110px_repeat(5,minmax(150px,1fr))] border-b border-border/65 bg-muted/20">
...
function TeacherScheduleCard({ teacher }: { teacher: TeacherScheduleItem }) {
  return (
    <section className="group overflow-hidden rounded-[20px] border border-border/70 bg-card shadow-[0_10px_30px_rgba(31,24,74,0.045)] transition-[border-color,box-shadow] duration-300 hover:border-[var(--academic-border)] hover:shadow-[var(--shadow-floating)]">
      {/* Teacher header */}
      <header className="relative overflow-hidden border-b border-border/65 px-5 py-4 sm:px-6">
        ...
        <div className="absolute -right-12 -top-16 h-40 w-40 rounded-full bg-[var(--academic-soft)] blur-3xl opacity-70" />
      ...
      <div className="space-y-3 p-4 md:hidden">
        {days.map((day) => {
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between border-b border-border/60 bg-[var(--academic-soft)] px-4 py-3">
                <span className="text-xs font-semibold text-[var(--academic-accent)]">
                  {day.label}
                </span>
...
function TeacherDesktopGrid({ teacher }: { teacher: TeacherScheduleItem }) {
    ...
    <>
      {Array.from({
        ...
        return (
          ...
          >
            {/* Period */}
            <div className="flex flex-col justify-center bg-muted/15 px-4 py-3">
              ...
              return (
                <div key={day.key} className="border-l border-border/60 p-2">
                  {entry ? (
                    ...
                    <div className="flex h-full min-h-[88px] items-center justify-center rounded-[16px] bg-muted/15 text-[11px] text-muted-foreground">
                      Free
                    </div>
                  )}
                </div>
...
function ScheduleEntry({ entry }: { entry: ScheduleEntryType }) {
  return (
    <div className="p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
        ...
        <span className="shrink-0 rounded-full bg-[var(--academic-soft)] px-3 py-1 text-[10px] font-semibold text-[var(--academic-accent)]">
          {entry.start_time || "—"}
        </span>
      </div>
...
function ScheduleCell({ entry }: { entry: ScheduleEntryType }) {
  return (
    <div className="min-h-[88px] rounded-[16px] border border-[var(--academic-border)] bg-[var(--academic-soft)]/45 p-3 transition-[background-color,transform] duration-200 hover:-translate-y-0.5 hover:bg-[var(--academic-soft)]">
