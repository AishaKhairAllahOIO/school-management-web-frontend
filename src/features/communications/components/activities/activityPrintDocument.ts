...
export function buildActivityPosterDocument(
  ...
  return createPosterDocument({
    ...
    identity,
    eyebrow: activity.type || "School activity",
    headline: activity.activity_name,
    description:
    ...
    details: [
      ...
      {
        label: "Time",
        value: `${formatTime(activity.start_time)} – ${formatTime(activity.end_time)}`,
      },
      {
        label: "Audience",
        value: activity.class_room_ids?.length
          ? `${activity.class_room_ids.length} classroom${activity.class_room_ids.length === 1 ? "" : "s"}`
          : "School community",
      },
    ],
    footer: "Take part · Create memories · Grow together",
    tone: "sky",
  });
}
