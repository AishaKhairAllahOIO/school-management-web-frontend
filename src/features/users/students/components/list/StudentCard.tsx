...
export function StudentCard({
  ...
  const isEnabled =
    normalizedAccountStatus === "enabled" ||
      ...
      <div className="flex min-h-0 flex-1 flex-col p-3.5">
        <div className="flex items-start justify-between gap-3">
          <button
            ...
            className="min-w-0 flex-1 rounded-xl text-left focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/10"
          >
          ...
          <button
            ...
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-primary/[0.08] hover:text-primary focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/10"
          >
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <InfoRow
            ...
            label="Grade"
            value={formatText(student.grade?.name)}
          />
          <InfoRow
            icon={<GraduationCap className="h-4 w-4" />}
            label="Classroom"
            value={formatText(student.classroom?.name)}
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 border-t border-primary/15 bg-primary/[0.025] px-3.5 py-2.5">
        {isDeleted ? (
          ...
          <>
            ...
            >
              {isToggling ? <Spinner /> : <Power className="h-4 w-4" />}
            </ActionButton>{" "}
          </>
        )}
      </div>
...
function AvatarFallback({ fullName }: { fullName: string }) {
  return (
    <div
      aria-hidden="true"
      className="flex h-12 w-12 shrink-0 items-center justify-center rounded-[15px] bg-primary/[0.08] text-primary"
    >
      <span className="text-sm font-medium">{getInitials(fullName)}</span>
      <UserRound className="hidden h-6 w-6" />
    </div>
  );
}
...
function InfoRow({
  ...
  return (
    <div className="flex min-w-0 items-center gap-2.5 rounded-2xl border border-border/55 bg-muted/30 px-3 py-2.5">
      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-primary/[0.08] text-primary shadow-[var(--shadow-soft)]">
        {icon}
      </span>
...
function ActionButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={[
        "inline-flex h-10 min-w-0 items-center justify-center gap-1.5 rounded-xl border border-border bg-card/80 px-2 text-[11px] font-semibold text-primary transition-colors hover:border-primary/20 hover:bg-primary/[0.08] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/10 disabled:cursor-not-allowed disabled:opacity-40",
        className,
      ].join(" ")}
    >
      {children}
