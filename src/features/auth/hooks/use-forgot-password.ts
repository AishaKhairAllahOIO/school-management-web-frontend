...
export function useForgotPassword() {
  ...
  return useMutation({
    ...
    onSuccess: (response, variables) => {
      toast.success(response.data.message || "Password reset code sent successfully.");
      navigate(AUTH_ROUTES.VERIFY_OTP, {
        replace: true,
        state: { email: variables.email, isResetFlow: true, remainingTime: response.data.data?.remaining_time },
      });
    },
