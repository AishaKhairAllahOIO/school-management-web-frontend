...
function getPaymentStatusLabel(status: import("../types/finance.types").PaymentStatus) {
  switch (status) {
    case "fully_paid":
      return "Paid in full";
    case "partially_paid":
...
export function StudentFinancialProfilePage() {
  ...
  const isFullyPaid = account?.paymentStatus === "fully_paid";
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-0.5 flex items-center gap-2 text-[10.5px] text-muted-foreground">
                <span>{account ? `Academic year ${account.academicYearId}` : "Financial account"}</span>
                {account ? (
                  <>
                    <span className="h-1 w-1 rounded-full bg-border" />
                    <span>{account.installmentPolicy?.name ?? "Not assigned"}</span>
                  </>
                ) : null}
              </div>
            </div>
