...
function ReceiptBody({ payment }: { payment: PaymentReceipt }) {
  const reference =
    payment.paperReceiptNo || payment.digitalReference || "Not provided";
    ...
    <div id="finance-payment-receipt" className="space-y-5">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-[11.5px] text-muted-foreground/75">
                Recorded on{" "}
                {formatDate(payment.createdAt ?? undefined)}
              </p>
            </div>
          </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <DetailItem
          icon={UserRound}
          label="Student"
          value="Not supplied by the backend"
        />
        <DetailItem
          icon={CreditCard}
        ...
        <DetailItem
          icon={CalendarDays}
          label="Payment date"
          value={formatDate(payment.createdAt ?? undefined)}
        />
        <DetailItem
        ...
        <DetailItem
          icon={Banknote}
          label="Applied to"
          value="Student financial account"
        />
      </div>
...
export function PaymentReceiptDialog({ open, onOpenChange, paymentId }: Props) {
  const paymentQuery = useFinancePayment(paymentId ?? undefined, open);
  ...
  function printReceipt() {
    ...
    printPreview.openPreview(
      buildPaymentReceiptDocument(paymentQuery.data, printIdentity),
    );
  }
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-h-[92vh] overflow-y-auto rounded-[28px] border-border/45 bg-background/95 p-0 shadow-[0_24px_80px_rgba(31,22,73,0.14)] backdrop-blur-xl sm:max-w-[720px]">
            ...
            <div className="flex items-start gap-3 pe-9">
              ...
              <div className="min-w-0 flex-1">
                ...
                <DialogDescription className="mt-0.5 text-[12px] font-normal text-muted-foreground/75">
                  Review the recorded payment details and print an official
                  copy.
                </DialogDescription>
              </div>
            </div>
            ...
            {paymentQuery.isLoading ? (
              ...
              <div className="rounded-[18px] border border-destructive/18 bg-destructive/[0.04] px-5 py-10 text-center">
                ...
                <p className="mt-1.5 text-[12px] text-muted-foreground">
                  The payment details request failed. Retry without closing the
                  dialog.
                </p>
                <Button
                  type="button"
