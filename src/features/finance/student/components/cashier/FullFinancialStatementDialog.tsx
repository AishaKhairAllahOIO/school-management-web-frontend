          ...
          <DialogHeader className="border-b border-border/40 px-5 py-5 text-start sm:px-6">
            <div className="flex items-start gap-3 pe-9">
              ...
              <div>
                ...
                <DialogDescription>
                  Contract, installments, and all recorded payments in one
                  printable document.
                </DialogDescription>
              </div>
            </div>
          ...
          <div className="p-5 sm:p-6">
            <div id="student-financial-statement" className="space-y-6">
              <div className="rounded-[20px] border border-primary/14 bg-primary/[0.035] p-5">
                ...
                <h2 className="mt-1 text-[21px] font-semibold text-foreground/92">
                  {studentName}
                </h2>
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                <div className="rounded-[16px] border border-border/45 p-4">
                  ...
                  <p className="mt-1 text-[19px] font-semibold">
                    {money(account.totalRequiredAmount)}
                  </p>
                </div>
              ...
              <section>
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                  <div className="rounded-[14px] border border-border/40 p-3">
                    ...
                    <tbody>
                      {account.installments.map((item) => (
                        <tr key={item.id} className="border-t border-border/35">
                          ...
                          <td className="p-3">{dateLabel(item.dueDate)}</td>
                          <td className="p-3">{money(item.amountDue)}</td>
                          <td className="p-3">{money(item.amountPaid)}</td>
                    ...
                    <table className="w-full min-w-[620px] text-left text-[12px]">
                      ...
                      <tbody>
                        {visiblePayments.length ? (
                          visiblePayments.map((payment: PaymentReceipt) => (
                            <tr
                              key={payment.id}
                          ...
                          <tr>
                            <td
                              ...
                              colSpan={5}
                            >
                              No payment records were returned for this account.
                            </td>
                          </tr>
                        )}
                      </tbody>
            ...
            <div className="mt-5 flex justify-end border-t border-border/40 pt-4">
              <Button
                ...
                className="rounded-[12px] border-success/20 text-success hover:bg-success/[0.055]"
              >
