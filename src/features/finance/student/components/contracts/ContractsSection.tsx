...
import type { FinancialAccount } from "../../../student/types/finance.types";
...
export function ContractsSection({
  ...
  const listedProfileByStudentId = useMemo(() => {
    ...
    return new Map(
      visibleListedStudents.map(
        (student, index) =>
          [String(student.studentId), listedProfiles[index]?.data] as const,
      ),
    );
  ...
  const detailByStudentId = useMemo(
    () =>
      new Map(
        unionStudentIds.map(
          (id, index) => [id, studentDetailQueries[index]?.data] as const,
        ),
      ),
  ...
  const { data: installmentPolicies = [], isLoading: isLoadingPolicies } =
    ...
    updateContract.mutate(
      {
        ...
        payload: {
          ...
          academicYearId: Number(
            selectedAccountToEdit?.academicYearId ?? 0,
          ),
          feePlanId: Number(values.feePlanId),
          installmentPolicyId: Number(values.installmentPolicyId),
          selectedExtraServiceIds:
            selectedAccountToEdit?.feePlan?.extraServices
              ?.map((service) => Number(service.id))
              .filter((id) => Number.isFinite(id)) ?? null,
        },
      },
      {
        onSuccess: () => {
  ...
  const dialogStudents = createStudent
    ...
    : students.filter((student) => !accountByStudentId.has(String(student.id)));
