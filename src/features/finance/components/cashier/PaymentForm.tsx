    ...
    <form onSubmit={handleSubmit((data) => onSubmit(data as PaymentFormValues))} className="space-y-6">
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        ...
        <div className="space-y-2 md:col-span-2">
              ...
              <Select value={field.value ? String(field.value) : ""} onValueChange={(val) => field.onChange(Number(val))}>
                ...
                <SelectContent className="rounded-xl border-border bg-popover text-popover-foreground shadow-lg">
                  {students.map((student) => (
                    <SelectItem key={student.id} value={String(student.id)} className="rounded-lg cursor-pointer">
                      {student.name}
                    </SelectItem>
                  ))}
                </SelectContent>
          ...
          />
          {errors.studentId && <p className="text-xs font-medium text-destructive">{String(errors.studentId.message)}</p>}
        </div>
        ...
        <div className="space-y-2">
          ...
          />
          {errors.paymentMethod && <p className="text-xs font-medium text-destructive">{String(errors.paymentMethod.message)}</p>}
        </div>
      ...
      <Button type="submit" className="primary-gradient h-12 w-full rounded-xl text-base font-semibold text-primary-foreground shadow-md transition-all hover:opacity-95 active:scale-[0.99]" disabled={isLoading}>
