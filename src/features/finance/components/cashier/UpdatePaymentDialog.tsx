      ...
      <DialogContent className="rounded-[28px] border-border/45 bg-background/95 p-0 shadow-[0_24px_80px_rgba(31,22,73,0.16)] backdrop-blur-xl sm:max-w-md">
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-5 px-6 pb-6 pt-5 sm:px-7">
          ...
          <div className="soft-purple-gradient flex items-center justify-between rounded-2xl border border-primary/20 p-4 text-sm">
            ...
            <span className="text-lg font-bold text-primary">{payment?.paidAmount?.toLocaleString()} $</span>
          </div>
              ...
              render={({ field }) => (
                <Select value={field.value as string} onValueChange={field.onChange}>
                  ...
                  <SelectContent className="rounded-xl border-border bg-popover text-popover-foreground shadow-lg">
                    <SelectItem value="cash" className="rounded-lg cursor-pointer">Cash</SelectItem>
                    <SelectItem value="bank_transfer" className="rounded-lg cursor-pointer">Bank Transfer</SelectItem>
                    <SelectItem value="cheque" className="rounded-lg cursor-pointer">Cheque</SelectItem>
                    <SelectItem value="electronic_wallet" className="rounded-lg cursor-pointer">E-Wallet</SelectItem>
                  </SelectContent>
                </Select>
              )}
            />
          ...
          <div className="space-y-2">
            <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Paper Receipt No.</label>
            <Input className="h-11 rounded-xl border-border bg-card text-foreground focus-visible:ring-ring" {...register("paperReceiptNo")} />
          </div>

          {selectedMethod !== "cash" && (
            <div className="space-y-2">
              <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Digital Reference</label>
              <Input className="h-11 rounded-xl border-border bg-card text-foreground focus-visible:ring-ring" {...register("digitalReference")} />
            </div>
          )}

          <Button type="submit" className="primary-gradient h-11 w-full rounded-xl font-semibold text-primary-foreground shadow-sm transition-all hover:opacity-95" disabled={isLoading}>
