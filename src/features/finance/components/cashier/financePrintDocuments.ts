...
export function buildPaymentReceiptDocument(
  ...
  const content = `
    <section class="official-title-block">
      <div>
        ...
        <h1 class="official-title">${escapePrintHtml(payment.studentName || "Student payment")}</h1>
    ...
    <section class="official-grid">
      <div class="official-field">
        <span>Student</span>
        <strong>${escapePrintHtml(payment.studentName || "—")}</strong>
      </div>
      <div class="official-field">
      ...
      <div class="official-field">
        <span>Cashier</span>
        <strong>${escapePrintHtml(payment.cashierName || "—")}</strong>
      </div>
      <div class="official-field">
        <span>Applied to</span>
        <strong>${escapePrintHtml(payment.installmentTitle || "Student financial account")}</strong>
      </div>
    </section>
    ...
    ${
      payment.notes
        ? `<section class="official-section">
             <h2 class="official-section-title">Notes</h2>
             <p class="official-description">${escapePrintHtml(payment.notes)}</p>
           </section>`
        : ""
    }
  ...
  return createOfficialDocument({
    title: `Payment receipt - ${payment.studentName || "Student"}`,
    identity,
...
export function buildFinalStatementDocument({
  ...
  const statementStatus =
    remaining <= 0 ? "Paid in full" : account.paymentStatus === "partially_paid"
      ? "Partially paid"
      : "Outstanding";
  ...
  const paymentRows = visiblePayments.length
    ? visiblePayments
        .map(
          (item) => `
            <tr>
              <td>${escapePrintHtml(dateLabel(item.paymentDate || item.createdAt))}</td>
              <td>${escapePrintHtml(paymentMethodLabel(item.paymentMethod))}</td>
              <td>${escapePrintHtml(money(item.paidAmount))}</td>
              <td>${escapePrintHtml(item.installmentTitle || "Financial account")}</td>
            </tr>`,
        )
        .join("")
  ...
  const content = `
    <section class="official-title-block">
      <div>
        ...
        <p class="official-description">
          ${escapePrintHtml(academicYearName || account.academicYearName || "Academic year")}
