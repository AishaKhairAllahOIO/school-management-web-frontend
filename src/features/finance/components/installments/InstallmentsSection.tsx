...
function ProgressBar({
  ...
  const progress =
    due > 0
      ? Math.min(100, Math.max(0, Math.round((paid / due) * 100)))
      : 0;
    ...
    return (
      <div className="rounded-[20px] border border-dashed border-border/55 bg-card px-6 py-14 text-center">
        ...
        <p className="mx-auto mt-1.5 max-w-md text-[12.5px] leading-5 text-muted-foreground/75">
          Installments will appear here once the student's
          financial contract has been finalized.
        </p>
      </div>
    );
  }
      ...
      >
        <div className="flex flex-col items-center justify-center rounded-[18px] border border-destructive/20 bg-destructive/[0.045] py-12 text-center">
          ...
          <p className="mt-1 max-w-sm text-[12px] leading-5 text-muted-foreground/70">
            Please try again to load the student's installment
            schedule.
          </p>
