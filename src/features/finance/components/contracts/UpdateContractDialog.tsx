...
const updateContractSchema = z.object({
  feePlanId: z.coerce.number().min(1, "Please select the installment amount."),
...
export function UpdateContractDialog({
  ...
  const { handleSubmit, setValue, watch, reset, formState: { errors } } =
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="overflow-hidden rounded-[28px] border-border/45 bg-background/95 p-0 shadow-[0_24px_80px_rgba(31,22,73,0.16)] backdrop-blur-xl sm:max-w-[540px]">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 pe-8">
                ...
                <DialogDescription className="mt-1 text-[12px] leading-5 text-muted-foreground/75">
                  Only the installment amount and payment policy can be changed. Student and academic year remain locked.
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
            ...
            <div className="relative mt-5 flex items-center gap-3 rounded-[18px] border border-border/40 bg-card/75 px-4 py-3 shadow-[0_8px_25px_rgba(31,22,73,0.035)]">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5 truncate text-[12.5px] font-medium text-foreground/80">
                  {account.feePlan?.name || "No fee plan"} · {account.installmentPolicy?.name || "No policy"}
                </p>
              </div>
            </div>
          ) : null}
        ...
        {account ? (
          <form onSubmit={handleSubmit(submit)} className="space-y-4 px-6 pb-6 sm:px-7 sm:pb-7">
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-[20px] border border-border/45 bg-card p-4 shadow-[0_8px_24px_rgba(31,22,73,0.035)]">
                <div className="mb-3 flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-[12px] font-semibold text-foreground/85">Installment amount</p>
                    <p className="text-[10px] text-muted-foreground/65">Fee plan</p>
                  </div>
                </div>

                <Select
                  value={feePlanId ? String(feePlanId) : ""}
                  onValueChange={(value) => setValue("feePlanId", Number(value), { shouldValidate: true })}
                >
                  ...
                  <SelectContent className="rounded-[14px]">
                    {feePlans.map((plan) => (
                      <SelectItem key={plan.id} value={String(plan.id)} className="rounded-[10px]">
                        {plan.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
                ...
                {errors.feePlanId ? (
                  <p className="mt-1.5 text-[10.5px] text-destructive">{errors.feePlanId.message}</p>
                ) : null}
              </div>
              ...
              <div className="rounded-[20px] border border-border/45 bg-card p-4 shadow-[0_8px_24px_rgba(31,22,73,0.035)]">
                <div className="mb-3 flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-[12px] font-semibold text-foreground/85">Payment policy</p>
                    <p className="text-[10px] text-muted-foreground/65">Installment schedule</p>
                  </div>
                </div>

                <Select
                  value={installmentPolicyId ? String(installmentPolicyId) : ""}
                  onValueChange={(value) => setValue("installmentPolicyId", Number(value), { shouldValidate: true })}
                >
                  <SelectTrigger className="h-11 rounded-[13px] border-border/50 bg-background/80 text-[12px] shadow-none focus:ring-2 focus:ring-primary/10">
                  ...
                  <SelectContent className="rounded-[14px]">
                    {installmentPolicies.map((policy) => (
                      <SelectItem key={policy.id} value={String(policy.id)} className="rounded-[10px]">
                        {policy.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
                ...
                {errors.installmentPolicyId ? (
                  <p className="mt-1.5 text-[10.5px] text-destructive">{errors.installmentPolicyId.message}</p>
                ) : null}
              </div>
            </div>
            ...
            <div className="flex items-start gap-2.5 rounded-[16px] border border-success/15 bg-success/[0.045] px-3.5 py-3 text-[10.5px] leading-5 text-muted-foreground/75">
              ...
              <span>The existing student, enrollment, academic year and payment history are not changed.</span>
            </div>

            <DialogFooter className="gap-2 border-t border-border/35 pt-4 sm:justify-end">
              <Button type="button" variant="ghost" onClick={() => onOpenChange(false)} className="h-10 rounded-[13px] px-4 text-[11.5px] text-muted-foreground hover:bg-muted/50">
                Cancel
              </Button>
              <Button type="submit" disabled={isLoading} className="h-10 rounded-[13px] bg-primary px-5 text-[11.5px] font-semibold text-primary-foreground shadow-[0_8px_20px_rgba(99,78,181,0.18)] hover:bg-primary/90">
