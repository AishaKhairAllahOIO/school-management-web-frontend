import { Bell, BookOpen, Building2, CalendarDays, FileText, LockKeyhole, ReceiptText, ShieldCheck, UserCheck, Users, Wallet, Banknote, type LucideIcon } from "lucide-react";
...
const subNavigationSections: SubNavigationSection[] = [
  ...
  { basePath: "/finance", items: [
    ...
    { title: "Staff Payroll", path: "/finance/payroll", icon: Banknote },
  ]},
...
function TabItem({ title, path, icon: Icon }: SubNavigationItem) {
  ...
  return <NavLink to={path} className={({ isActive }) => [
    ...
    staffPayroll ? "focus-visible:ring-info/10" : "focus-visible:ring-primary/10",
    isActive ? (staffPayroll ? "bg-info/[0.09] text-info" : "bg-primary/[0.07] text-primary") : "text-muted-foreground hover:bg-muted/40 hover:text-foreground",
  ].join(" ")}>
    {({ isActive }) => <><Icon aria-hidden size={17} strokeWidth={1.8} className={isActive ? (staffPayroll ? "shrink-0 text-info" : "shrink-0 text-primary") : "shrink-0 text-muted-foreground transition-colors group-hover:text-foreground"}/><span className="whitespace-nowrap">{title}</span><span aria-hidden className={["absolute bottom-0 left-4 right-4 h-[2px] origin-center rounded-full transition-transform duration-200", staffPayroll ? "bg-info" : "bg-primary", isActive ? "scale-x-100" : "scale-x-0"].join(" ")}/></>}
  </NavLink>;
}

function DefaultSubNavigation({ items }: { items: SubNavigationItem[] }) {
  return <nav aria-label="Section navigation" className="w-full min-w-0 overflow-hidden rounded-[22px] border border-border/60 bg-card/95 p-1.5 shadow-[0_8px_28px_rgba(38,24,84,0.045)] backdrop-blur-sm"><div className="flex h-11 w-full min-w-0 items-center gap-1 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{items.map(item=><TabItem key={item.path} {...item}/>)}</div></nav>;
}
