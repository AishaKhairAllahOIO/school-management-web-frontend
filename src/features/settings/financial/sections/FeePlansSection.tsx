...
type Props = {
  ...
  gradeLevels: Option[];
  // تم حذف installmentPolicies من هنا
};

export function FeePlansSection({ academicYears, gradeLevels }: Props) {
  const { data: feePlans = [], isLoading, isError, createFeePlan, updateFeePlan, deleteFeePlan } = useFeePlans();
  ...
  if (isError) return <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-red-600">Failed to load Fee Plans.</div>;
