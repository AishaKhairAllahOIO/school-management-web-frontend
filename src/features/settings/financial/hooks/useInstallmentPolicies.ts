...
export function useInstallmentPolicies() {
  ...
  const updatePolicy = useMutation({
    ...
    onError: (error: any) => {
      ...
      alert(
        "الباك إند رفض التعديل!\nالسبب: " +
          (serverMessage ||
            "تأكد هل الراوت POST أو PUT في api.php")
      );
  ...
  const deletePolicy = useMutation({
    mutationFn: (id: number) =>
      financialService.deleteInstallmentPolicy(id),
    ...
    onError: (error: any) => {
      ...
      alert(
        "الباك إند رفض الحذف!\nالسبب: " +
          (serverMessage ||
            "غالباً السياسة مرتبطة بخطة رسوم ولا يمكن حذفها.")
      );
  ...
  /**
   * Get one installment policy by ID
   */
  const getInstallmentPolicy = async (id: number) => {
    return financialService.getInstallmentPolicy(id);
  };
