...
export function AdviserDashboard() {
  return (
    <div className="space-y-5">
      ...
      <section className="grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
        <DashboardSection title="Students Requiring Follow-up">
          <div className="space-y-3">
            {[
              ...
              <div key={name} className="flex items-center justify-between rounded-[16px] bg-background/70 p-4">
                <div>
        ...
        <DashboardSection title="Quick Actions">
          <div className="grid grid-cols-2 gap-3">
            <button className="rounded-[16px] bg-primary/10 p-4 text-xs font-medium text-primary">
              <ShieldCheck className="mx-auto mb-2" size={22} />
              Open Case
            </button>

            <button className="rounded-[16px] bg-warning/10 p-4 text-xs font-medium text-warning">
              <AlertTriangle className="mx-auto mb-2" size={22} />
              Add Report
            </button>

            <button className="rounded-[16px] bg-info/10 p-4 text-xs font-medium text-info">
              <CalendarDays className="mx-auto mb-2" size={22} />
              Schedule
            </button>

            <button className="rounded-[16px] bg-success/10 p-4 text-xs font-medium text-success">
      ...
      <DashboardSection title="Adviser Activity">
        <div className="space-y-4">
          {supervisorActivities.map((item) => (
            <div key={item.id} className="flex items-start gap-3">
              <span className="mt-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-[16px] bg-primary/10 text-primary">
