  ...
  return <div className="space-y-4">
    <section className="flex flex-col gap-3 rounded-[22px] border border-border/45 bg-card p-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex gap-2">
        <button onClick={handleExport} className="inline-flex h-9 items-center gap-2 rounded-full border border-border/65 px-4 text-[11px] font-medium text-foreground/75 hover:bg-muted/40"><Download size={14}/>Export</button>
        <button onClick={openCreate} className="inline-flex h-9 items-center gap-2 rounded-full bg-primary px-4 text-[11px] font-medium text-primary-foreground"><Plus size={14}/>Add Exam</button>
      </div>
    </section>
    ...
    <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
      {exams.map((exam, index) => <article key={exam.id} className={["rounded-[22px] border p-4", ["border-violet-200/60 bg-violet-50/60","border-sky-200/60 bg-sky-50/60","border-emerald-200/60 bg-emerald-50/60"][index%3]].join(" ")}>
        <div className="flex items-start justify-between gap-3"><div><p className="text-[13px] font-medium text-foreground">{formatSubject(exam.subject)}</p><p className="mt-1 text-[10px] text-muted-foreground">{grade} grade</p></div><ScheduleStatusBadge status={exam.status}/></div>
        <div className="mt-4 grid gap-2 text-[10px] text-muted-foreground sm:grid-cols-2"><Info icon={CalendarDays} text={exam.date}/><Info icon={Clock3} text={`${exam.startTime} · ${exam.duration}`}/><Info icon={MapPin} text={exam.room}/></div>
...
function Action({icon:Icon,label,onClick,destructive=false}:{icon:typeof Edit3;label:string;onClick:()=>void;destructive?:boolean}) { return <button onClick={onClick} className={`inline-flex h-8 items-center gap-1.5 rounded-full border px-3 text-[10px] font-medium ${destructive?"border-rose-200 bg-rose-50 text-rose-600":"border-border/60 bg-card text-foreground/70"}`}><Icon size={12}/>{label}</button>; }

export function Editor({title,onClose,onSave,children}:{title:string;onClose:()=>void;onSave:()=>void;children:ReactNode}) { return <div className="fixed inset-0 z-[140] flex items-center justify-center bg-slate-950/30 p-4 backdrop-blur-[5px]" onMouseDown={(e)=>e.target===e.currentTarget&&onClose()}><div className="w-full max-w-xl overflow-hidden rounded-[24px] border border-border/55 bg-card shadow-[0_28px_90px_rgba(15,10,40,0.22)]"><header className="flex items-center justify-between border-b border-border/45 px-5 py-4"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-[13px] bg-primary/[0.09] text-primary"><FileText size={17}/></span><h2 className="text-[16px] font-medium">{title}</h2></div><button onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-[11px] border border-border/60"><X size={15}/></button></header><div className="grid gap-4 p-5 sm:grid-cols-2">{children}</div><footer className="flex justify-end gap-2 border-t border-border/45 px-5 py-4"><button onClick={onClose} className="h-9 rounded-full border border-border/65 px-4 text-[11px]">Cancel</button><button onClick={onSave} className="h-9 rounded-full bg-primary px-4 text-[11px] text-primary-foreground">Save</button></footer></div></div>; }
export function Input({label,value,onChange,type="text"}:{label:string;value:string;onChange:(v:string)=>void;type?:string}) { return <label><span className="mb-1.5 block text-[11px] font-medium text-foreground/80">{label}</span><input type={type} value={value} onChange={(e)=>onChange(e.target.value)} className="h-11 w-full rounded-[13px] border border-border/70 bg-background px-3.5 text-sm font-normal outline-none focus:border-primary/45 focus:ring-4 focus:ring-primary/10"/></label>; }
export function SelectField({label,value,onChange,options}:{label:string;value:string;onChange:(v:string)=>void;options:{value:string;label:string}[]}) { return <label><span className="mb-1.5 block text-[11px] font-medium text-foreground/80">{label}</span><select value={value} onChange={(e)=>onChange(e.target.value)} className="h-11 w-full rounded-[13px] border border-border/70 bg-background px-3.5 text-sm font-normal outline-none focus:border-primary/45 focus:ring-4 focus:ring-primary/10">{options.map(o=><option key={o.value} value={o.value}>{o.label}</option>)}</select></label>; }
export function ConfirmDelete({title,description,onClose,onConfirm}:{title:string;description:string;onClose:()=>void;onConfirm:()=>void}) { return <div className="fixed inset-0 z-[150] flex items-center justify-center bg-slate-950/30 p-4 backdrop-blur-[5px]"><div className="w-full max-w-md rounded-[24px] border border-border/55 bg-card p-5"><span className="flex h-11 w-11 items-center justify-center rounded-[16px] bg-rose-50 text-rose-600"><Trash2 size={19}/></span><h2 className="mt-4 text-[16px] font-medium">{title}</h2><p className="mt-1.5 text-[11px] leading-5 text-muted-foreground">{description}</p><div className="mt-5 flex justify-end gap-2 border-t border-border/45 pt-4"><button onClick={onClose} className="h-9 rounded-full border border-border/65 px-4 text-[11px]">Cancel</button><button onClick={onConfirm} className="h-9 rounded-full bg-destructive px-4 text-[11px] text-destructive-foreground">Delete</button></div></div></div>; }
