...
const gradeToneClasses = [
  pastelByColor.violet,
  pastelByColor.sky,
  pastelByColor.mint,
  pastelByColor.peach,
  pastelByColor.rose,
...
export function ClassSchedulesPage() {
  ...
  const classroomOptions = useMemo(
    () =>
      classrooms.filter(
        (item) => item.gradeId === activeGradeId,
      ),
    [classrooms, activeGradeId],
  );
  ...
  const classroomMap = new Map(
    classrooms.map((item) => [item.id, item]),
  );

  const gradeMap = new Map(
    grades.map((item) => [item.id, item]),
  );
  ...
  if (grades.length === 0) {
    return (
      <section className="rounded-[26px] border border-border/55 bg-card p-8 text-center">
        <p className="text-sm font-medium text-foreground">
          No grades are available yet.
        </p>

        <p className="mt-2 text-[12px] text-muted-foreground">
          Add grades from Academics before creating class schedules.
        </p>
      </section>
    );
  }
  ...
  return (
    <div className="space-y-5">
              ...
              <SelectContent className="rounded-[14px] border-border/60 p-1.5">
                {classroomOptions.map((item) => (
                  <SelectItem key={item.id} value={item.id} className="rounded-[10px] font-normal">
                    {item.name} · {item.currentStudentsCount} students
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
        ...
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-[12px] text-muted-foreground">
              Capacity {selectedClassroom?.capacity ?? "—"} · {selectedClassroom?.currentStudentsCount ?? 0} students
            </p>
          </div>
              ...
              {weekDays.map((day) => {
                ...
                return <div key={day} className="min-h-[112px] border-r border-border/45 p-2 last:border-r-0">
                  {schedule ? <article className={`group h-full rounded-[15px] border p-2.5 ${subject ? pastelByColor[subject.color] : "border-border/55 bg-muted/15"}`}>
                    ...
                    <p className="mt-2 truncate text-[10px] text-foreground/75">{teacher?.name}</p><p className="mt-1 text-[10px] text-muted-foreground">
      ...
      <ClassScheduleFormDialog
        ...
        classroomOptions={classrooms.map((item) => ({
          value: item.id,
